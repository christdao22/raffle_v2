          ...
          <div>
            <div className="flex items-center justify-center gap-3">
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-tr-primary-container/40 bg-tr-surface-container/80 px-4 py-1.5 shadow-sm backdrop-blur-md">
                <span className="font-display text-lg font-black leading-none text-tr-primary sm:text-xl">
                  {count}
                </span>
            ...
            {currentPrize?.imageUrl && (
              <div className="group relative mx-auto mt-5 w-full overflow-hidden rounded-2xl border border-white/80 bg-linear-to-br from-white/80 via-white/55 to-tr-primary/15 px-5 text-center shadow-[0_18px_40px_rgba(6,38,141,0.18)] backdrop-blur-xl transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_24px_48px_rgba(6,38,141,0.24)] sm:px-8 sm:py-5">
                ...
                <h2 className="relative font-display wrap-break-word mx-auto max-w-full text-3xl font-black uppercase leading-[1.05] tracking-tight text-tr-secondary drop-shadow-[0_3px_10px_rgba(6,38,141,0.22)] sm:text-4xl lg:text-5xl xl:text-6xl">
                  {prizeName}
                </h2>
              </div>
            )}
          ...
          <div className="grid min-h-0 grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:gap-8">
            {sponsorName && (
              <div className="order-2 flex flex-col items-center justify-center text-center lg:order-1">
                <span className="font-sans text-xs font-bold uppercase tracking-[0.2em] text-tr-primary/90 sm:text-sm">
                  Sponsored By
                </span>
                {currentPrize.sponsorImage && (
                  <div className="flex h-24 w-48 items-center justify-center overflow-hidden rounded-xl bg-white p-3 sm:h-28 sm:w-56 lg:h-32 lg:w-64 xl:h-80 xl:w-100">
                    <img
                ...
                <span className="font-display wrap-break-word max-w-full text-2xl font-black text-tr-secondary sm:text-3xl lg:text-3xl">
                  {sponsorName}
                </span>
              </div>
            )}
              ...
              {currentPrize?.imageUrl ? (
                <>
                  ...
                  <img
                    ...
                    alt={prizeName}
                    decoding="async"
                    className="prize-float relative z-10 max-h-56 max-w-full object-contain px-3 drop-shadow-[0_20px_30px_rgba(0,0,0,0.5)] transition-transform duration-500 hover:scale-105 sm:max-h-90 xl:max-h-80"
                  />
                </>
              ) : (
                <div className="relative z-10 flex min-h-48 w-full max-w-xl items-center justify-center rounded-2xl border border-tr-primary-container/40 bg-linear-to-br from-tr-surface/80 to-tr-secondary-container/15 px-6 py-8 text-center shadow-[0_16px_35px_rgba(6,38,141,0.16)] backdrop-blur-xl">
                  <span className="font-display wrap-break-word text-3xl font-black uppercase leading-tight tracking-tight text-tr-secondary drop-shadow-[0_3px_10px_rgba(6,38,141,0.2)] sm:text-4xl lg:text-5xl">
                    {prizeName}
                  </span>
                </div>
              )}
